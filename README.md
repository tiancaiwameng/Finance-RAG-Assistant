...
5. DeepSeek 只依据检索证据回答，并用 `[来源1]` 格式引用。
...
- 金融约束 Prompt：区分客观数据、管理层陈述和未来计划，证据不足时拒答；
- 多轮对话：保留最近三轮语境，每个问题仍重新检索证据；
- 数据安全：API Key 只从 `.env` 读取，不写入代码或 Git 仓库。
...
启动项目后建议截取以下两张图并放入 `screenshots/`：

1. `screenshots/home.png`：上传示例 PDF 后的知识库统计；
2. `screenshots/answer-with-sources.png`：问答结果与展开的原文证据。

随后在本节加入：

```markdown
![项目首页](screenshots/home.png)
![带来源的回答](screenshots/answer-with-sources.png)
```

不直接提供合成截图，避免 GitHub 展示与真实运行效果不一致。
...
先在 GitHub 新建同名空仓库（不要勾选自动生成 README），然后在项目目录运行：

```bash
git init
git add .
git commit -m "feat: build finance RAG assistant"
git branch -M main
git remote add origin https://github.com/<your-username>/Finance-RAG-Assistant.git
git push -u origin main
```

推送前执行 `git status`，确认没有 `.env`、真实财报或客户敏感资料。建议为仓库添加 `rag`、`fintech`、`langchain`、`deepseek`、`streamlit`、`chromadb` Topics。

## 面试讲解

面试时不要只说“我调用了大模型 API”。建议用四句话讲清楚：

> 我发现财报和公告篇幅长，分析师定位经营原因与风险因素效率较低，所以做了一个金融文档 RAG 助手。系统逐页解析 PDF、进行中文文本切分并写入 Chroma，提问时先检索证据，再让 DeepSeek 严格基于证据回答。与普通聊天 Demo 不同，我保留了文件名、页码、相关度和原文片段，让每个结论可以回到公告复核。我还针对金融场景处理了数字口径、管理层计划与已实现业绩的区分，以及证据不足时的拒答。
