...
├── docs/
│   └── TECHNICAL_NOTES.md    # 系统设计、技术取舍与常见问题
└── screenshots/
...
Finance RAG Assistant 将金融 PDF 的输入校验、逐页解析、文本清洗与切分、本地向量化、Chroma 检索和 DeepSeek 生成串联为一条可复核的 RAG 流程。文件哈希贯穿文档处理和检索元数据，同一文档集合可以复用已有向量数据；页面保留文件名、页码、相关度和原文片段，便于核对生成内容。

工程侧重点包括输入质量、重复内容处理、关键步骤日志、分层异常和可选 MySQL 记录。Chroma 负责文本片段与语义向量检索，MySQL 负责文档处理状态和问答历史；数据库不可用时保留核心 RAG 服务并记录降级状态。当前项目仍是本地技术实现，不具备生产系统所需的 OCR、正式评测、权限隔离和完整监控能力。

更完整的系统设计、技术取舍与常见问题见 [`docs/TECHNICAL_NOTES.md`](docs/TECHNICAL_NOTES.md)。
